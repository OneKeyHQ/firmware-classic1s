...
static const EthereumNetworkInfo *get_network(
  ...
  uint64_t evm_chain_id = chain_id;
  ...
  if (encoded_network) {
    ...
    if (decode_definition(encoded_network->size, encoded_network->bytes,
      ...
      evm_chain_id = decoded_network.chain_id;
    }
  }
  // try to get built-in definition
  if (evm_chain_id != CHAIN_ID_UNKNOWN) {
    network = ethereum_get_network_by_chain_id(evm_chain_id);
