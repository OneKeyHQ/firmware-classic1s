...
static void layoutWelcome(int key, int index) {
  (void)key;
  char desc[64] = "";
  char index_str[16] = "";
  int len = 0;
...
void onboarding(uint8_t key) {
  ...
  switch (index) {
    case 0:
      ...
      break;
    case 1:
      layoutWelcome(key, welcome_index);
      break;
    case 2:
...
bool layoutConfirmTx(const CoinInfo *coin, AmountUnit amount_unit,
  ...
  oledClear();
  layoutHeader(title);
  oledDrawStringAdapter(0, 13, _("Total Amount:"), FONT_STANDARD);
  oledDrawStringAdapter(0, 13 + 10, str_out, FONT_STANDARD);
  layoutButtonNoAdapter(NULL, &bmp_bottom_left_close);
...
void layoutConfirmAutoLockDelay(uint32_t delay_ms) {
  ...
  strlcpy(line, _("device after "), sizeof(line));
  ...
  oledClear_ex();
  layoutHeader(_("Change Auto-Lock Time"));
  if (ui_language) {
    strcat(line_cn, "后自动锁");
...
refresh_menu:
  ...
  switch (index) {
    ...
    case 3:
      ...
      char sn[32] = {0}, st_sn[32] = {0};
      data2hex((uint8_t*)(0x1FFF7A10), 4, st_sn);
      se_get_sn(&se_sn);
      strcat(sn, se_sn);
      strcat(sn, st_sn);

      oledDrawStringAdapter(0, y, sn, FONT_STANDARD);
...
bool layoutSignHash(const char *chain_name, bool verify, const char *signer,
  ...
  strcat(domain_desc, "Domain");
  strcat(domain_desc, _("Hash"));
