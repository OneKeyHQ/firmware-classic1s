...
void config_init(void) {
  ...
  if (se_version) {
    ...
    layoutDialog(&bmp_icon_error, NULL, NULL, NULL, "Get SE version", "failed.",
                 NULL, NULL, "Contact our support.", NULL);
    delay_ms(1000);
    shutdown();
  }
...
static void get_u2froot_callback(uint32_t iter, uint32_t total) {
  layoutProgressAdapter(_(C__UPDATING), 1000 * iter / total);
}
  ...
  if (config_getMnemonic(mnemonic, sizeof(mnemonic))) {
    ...
    if (!imported) {
      ...
      if (!mnemonic_check(mnemonic)) {
        ...
        error_shutdown("Storage failure", "detected.", NULL, NULL);
      }
    }
    char oldTiny = usbTiny(1);
    ...
    if (derive_cardano) {
      ...
      if (mnemonic_bits_len == 0 || mnemonic_bits_len % 33 != 0) {
        fsm_sendFailure(FailureType_Failure_ProcessError, "Invalid mnemonic");
        return false;
      }
...
bool config_getRootNode(HDNode *node, const char *curve) {
  ...
  if (result == 0) {
    fsm_sendFailure(FailureType_Failure_NotInitialized, "Unsupported curve");
  }
  return result;
}
