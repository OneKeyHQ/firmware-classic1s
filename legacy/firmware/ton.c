...
bool ton_sign_message(const TonSignMessage *msg, const HDNode *node,
    ...
    if(msg->jetton_amount == 0) {
        ...
        if (msg->has_comment) {
            if (memcmp(msg->comment, "b5ee9c72", 8) == 0) {
                ...
                if (!layoutTransactionSign("Ton", 0, false, amount_str, msg->destination, usr_friendly_address, NULL, NULL, (const uint8_t*)raw_data, data_len,
        ...
        if (is_raw_data) {
            if (!ton_prase_boc(raw_data, data_len, payload)) {
                free(raw_data);
                fsm_sendFailure(FailureType_Failure_ProcessError, "Failed to create raw data body");
                return false;
            }
            free(raw_data);
        } else {
    ...
    if (msg->ext_destination_count > 0) {
        ext_dest_count = (msg->ext_destination_count <= 3) ? msg->ext_destination_count : 3;
    ...
    ton_create_message_digest(msg->expire_at, msg->seqno, parsed_dest.is_bounceable, parsed_dest.workchain, parsed_dest.hash,
                              msg->ton_amount, msg->mode, NULL, payload, 
