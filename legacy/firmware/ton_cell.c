...
bool build_message_ref(bool is_bounceable, uint8_t dest_workchain, uint8_t* dest_hash, 
                       uint64_t value, CellRef_t* init, CellRef_t* payload, CellRef_t* out_message_ref) {
    BitString_t bits;
    bitstring_init(&bits);
...
bool ton_create_message_digest(uint32_t expire_at, uint32_t seqno, bool is_bounceable, uint8_t dest_workchain, uint8_t* dest_hash, 
                               uint64_t value, uint8_t mode, CellRef_t* init, CellRef_t* payload, 
                               const char** ext_dest, const uint64_t* ext_ton_amount, const char** ext_payload, uint8_t ext_dest_count,
                               uint8_t* digest) {
    // Build Internal Message
    struct CellRef_t internalMessageRef;
    if (!build_message_ref(is_bounceable, dest_workchain, dest_hash, value, init, payload, &internalMessageRef)) {
        return false;
    }
    ...
    for (int i = 0; i < ext_dest_count && i < 3; i++) {
        ...
        if (!build_message_ref(parsed_addr.is_bounceable, (uint8_t)parsed_addr.workchain, parsed_addr.hash, 
                               ext_ton_amount[i], NULL, ext_payload[i] ? &ext_payload_ref : NULL, 
    ...
    int total_refs = 1;  // Start from 1 because there's always an internal message
...
void set_top_upped_array(uint8_t *array, size_t array_len, bool fullfilled_bytes, uint16_t *cursor) {
    *cursor = array_len * 8;  // Initialize cursor to the length of the array times 8

    if (fullfilled_bytes || array_len == 0) {
        return;  // If it's a fully filled byte or the array is empty, return directly
    }
...
bool ton_prase_boc(const uint8_t *input_boc, size_t input_boc_len, CellRef_t *payload) {
    if (input_boc_len < 5) {
    ...
    for (uint32_t i = 0; i < cells_num; i++) {
        ...
        set_top_upped_array(cell_data[i].bits.data, data_bytes, is_fullfilled_bytes, &data_cursor);
    ...
    for (int i = cells_num - 1; i >= 0; i--) {
        ...
        if (!ton_hash_cell(&cell_data[i].bits, refs, cell_data[i].refs_count, &cell_data[i].cell_ref)) {
